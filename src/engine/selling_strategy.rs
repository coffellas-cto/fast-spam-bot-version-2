...
impl SimpleSellingEngine {
    ...
    pub async fn execute_sell(&self, trade_info: &TradeInfoFromToken) -> Result<()> {
        ...
        let (token_amount_raw, token_amount, token_decimals) = match BOUGHT_TOKENS.get_cached_balance(&trade_info.mint, 30) {
            Some(cached) => {
                let (token_amount_raw, token_amount, token_decimals) = cached;
                self.logger.log(format!("Using cached balance for selling {} tokens ({} raw units)", 
                    token_amount, token_amount_raw).cyan().to_string());
                (token_amount_raw, token_amount, token_decimals)
            },
            None => {
                self.logger.log(format!("No cached balance found for {}, attempting to fetch fresh balance...", trade_info.mint).yellow().to_string());
                
                // Try to fetch fresh balance as fallback
                match self.get_token_balance_fast(&trade_info.mint, &token_info.token_account).await {
                    Ok((token_amount_raw, token_amount, token_decimals)) => {
                        self.logger.log(format!("Successfully fetched fresh balance for {}: {} tokens", 
                            trade_info.mint, token_amount).green().to_string());
                        (token_amount_raw, token_amount, token_decimals)
                    },
                    Err(e) => {
                        self.logger.log(format!("Failed to fetch fresh balance for {}: {}", trade_info.mint, e).red().to_string());
                        return Err(anyhow!("Cached balance required for selling but not found, and fresh fetch failed: {}", e));
                    }
                }
            }
        };
...
impl SimpleSellingEngine {
    ...
    async fn verify_and_cache_balance(&self, mint: &str, ata: &Pubkey) -> Result<()> {
        ...
        // Try multiple times with increasing delays since token account creation can be slow
        let mut attempts = 0;
